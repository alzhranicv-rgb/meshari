...
async function loadPresenterSession() {
  ...
  if (error) {
    console.log(error)
    showToast("تعذر قراءة جلسة العرض")
    return null
  }
...
function renderPresenterFromSession(session) {
  presenterModel = Number(session.model || 1)
...
function updatePresenterUI() {
  ...
  if (presenterSegment === "warmup") renderPresenterWarmup()
  if (presenterSegment === "top10") renderPresenterTop10()
  if (presenterSegment === "auction") renderPresenterAuction()
  if (presenterSegment === "who") renderPresenterWho()
  if (presenterSegment === "final") renderPresenterFinal()
  if (presenterSegment === "archive") renderPresenterArchive()
}
/* =========================
